...
pub extern "C" fn _start() -> ! {
    write(1, b"\n========================================\n");
    write(1, b"  Axiom sys_exit Hook Demo\n");
    write(1, b"  Live hook -> ringbuf -> userspace\n");
    write(1, b"========================================\n\n");
    ...
    write(1, b"[3/4] Attaching to sys_exit... ");
    let attach_attr = BpfAttr {
        attach_btf_id: ATTACH_TYPE_SYS_EXIT,
    ...
    while seen < 2 {
        ...
        if poll_res as usize >= SYSCALL_EXIT_CONTEXT_SIZE {
...
fn build_program(ringbuf_map_id: i32) -> [BpfInsn; 12] {
    [
        ...
        BpfInsn {
            ...
            dst_src: regs(3, 0),
            off: 0,
            imm: SYSCALL_EXIT_CONTEXT_SIZE as i32,
        },
        BpfInsn {
            code: 0xb7,
            dst_src: regs(4, 0),
...
fn parse_event(buf: &[u8; 32]) -> SysExitEvent {
    let syscall_nr = u64::from_ne_bytes([
        buf[0], buf[1], buf[2], buf[3], buf[4], buf[5], buf[6], buf[7],
    ]);
    let result = i64::from_ne_bytes([
        buf[8], buf[9], buf[10], buf[11], buf[12], buf[13], buf[14], buf[15],
    ]);
    SysExitEvent { syscall_nr, result }
}
